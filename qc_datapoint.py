"""
QC DataPoint LLM Analytics System
...
Requirements:
pip install anthropic pandas numpy scikit-learn faiss-cpu python-dotenv
"""
...
class QCDataPointAnalyzer:
    """
    Main class for analyzing QC DataPoint traffic data using OpenAI LLM
    """
    ...
    def load_intersection_directory(self, dir_path: str) -> pd.DataFrame:
        """
        Load traffic count data from a directory of intersection CSV files
        
        Args:
            dir_path: Path to directory with intersection CSVs
        ...
        data_map = {} # (station_id, date) -> record
        for filepath in glob.glob(os.path.join(dir_path, "*.csv")):
            ...
            for line in lines:
                ...
                elif parts[0] == "Latitude:": metadata['latitude'] = float(parts[1]) if parts[1] and parts[1] != '""' else 0.0
                elif parts[0] == "Longitude:": metadata['longitude'] = float(parts[1]) if parts[1] and parts[1] != '""' else 0.0
                
                # Detect start of any volume table
                ...
                if active_table:
                    ...
                    if parts[0] == "": continue
                    if parts[0].isupper() and "VOLUMES" not in parts[0] and ":" not in parts[0]:
                        active_table = False
                        continue
                    ...
                    if total_idx != -1 and parts[0] and parts[0] != '""':
                        try:
                            time_period = parts[0]
                            if not time_period or time_period == '""': continue
                            
                            total_vol = int(parts[total_idx]) if parts[total_idx] and parts[total_idx] != '""' else 0
                            
                            date_str = f"{metadata.get('date', '')} {time_period}"
                            dt = pd.to_datetime(date_str, format="%b %d %Y %I:%M %p", errors='coerce')
                            
                            record_id = (metadata.get('station_id', 'UNKNOWN'), dt)
                            
                            # Update existing record or create new one
                            if record_id not in data_map:
                                data_map[record_id] = {
                                    'station_id': record_id[0],
                                    'date': record_id[1],
                                    'roadway': metadata.get('roadway', ''),
                                    'lat': metadata.get('latitude', 0.0),
                                    'lng': metadata.get('longitude', 0.0),
                                    'volume': 0,
                                    'volume_heavy': 0,
                                    'volume_pedestrian': 0,
                                    'volume_bicycle': 0,
                            ...
                            if table_type == "ALL-VEHICLE VOLUMES":
                                data_map[record_id]['volume'] = total_vol
                            elif table_type == "HEAVY-VEHICLE VOLUMES":
                                data_map[record_id]['volume_heavy'] = total_vol
                            elif table_type == "PEDESTRIAN VOLUMES":
                                data_map[record_id]['volume_pedestrian'] = total_vol
                            elif table_type == "BICYCLE VOLUMES":
                                data_map[record_id]['volume_bicycle'] = total_vol
                                
                        except Exception as e:
                            pass
                            
        self.df = pd.DataFrame(list(data_map.values()))
        print(f"Loaded {len(self.df)} records")
        print(f"Columns: {list(self.df.columns)}")
        return self.df
    ...
    def csv_to_json_corpus(self, 
        ...
        """
        Convert CSV data to structured JSON corpus for LLM
        ...
        """
        print("Converting CSV to JSON corpus...")
        self.corpus = []
        ...
        for idx, row in self.df.iterrows():
            ...
            obs = {
                ...
                "station": {
                    ...
                    "roadway": str(row.get(roadway_col, "")) if roadway_col else "",
                },
                "temporal": {
                ...
                "metrics": {
                    ...
                    "volume_heavy": float(row.get('volume_heavy', 0)) if pd.notna(row.get('volume_heavy')) else 0,
                    "volume_pedestrian": float(row.get('volume_pedestrian', 0)) if pd.notna(row.get('volume_pedestrian')) else 0,
                    "volume_bicycle": float(row.get('volume_bicycle', 0)) if pd.notna(row.get('volume_bicycle')) else 0,
                },
                "qc": {
            ...
            if speed_mean_col and speed_mean_col in self.df.columns:
                obs["metrics"]["speed_mean"] = float(row.get(speed_mean_col, 0)) if pd.notna(row.get(speed_mean_col)) else None
            
            if speed_p85_col and speed_p85_col in self.df.columns:
                obs["metrics"]["speed_p85"] = float(row.get(speed_p85_col, 0)) if pd.notna(row.get(speed_p85_col)) else None
